#!/usr/bin/env python3
"""Capture rig, Claude Code half: drive the real interactive TUI in a pty.

The interactive CLI is the shape this fork reproduces, and it cannot be reached
with `claude -p`: `--print` sends `cc_entrypoint=sdk-cli`, the Agent SDK identity
line, and `cc_turn_origin=sdk`. Only the TUI sends `cli` / the CLI identity /
`human`. Driving it needs a pty, which Node cannot do without a native addon, so
this one script is Python. Any Python 3 works; macOS ships one.

Usage (with scripts/capture-requests.ts already listening):
