 ...
 *
 * Usage:
 *   node --experimental-strip-types scripts/verify-fingerprint.ts <file|dir>...
 *
 ...
 *   # checked when it is known or supplied.
 *   node --experimental-strip-types scripts/verify-fingerprint.ts capture.json \
...
for (const path of collect(targets)) {
    ...
    // native capture anything listed under "native-only" is drift this fork has
    // not tracked; on a pi capture the extras are pi's own feature betas.
    ...
    if (observed.length > 0) {
        ...
        const nativeOnly = expected.filter((beta) => !observed.includes(beta))
        const forkOnly = observed.filter((beta) => !expected.includes(beta))
        console.log(
            `  betas observed=${observed.length} missing=${JSON.stringify(nativeOnly)} extra=${JSON.stringify(forkOnly)}`,
        )
    }
    console.log()
}
