 ...
 * request. That second request is where native `cc_prev_req` (and mid-conversation
 * system traffic) appear.
 *
 * Then point a client at it. For this fork's production mission the native
 * reference is **`claude -p` / sdk-cli**, not the interactive TUI:
 *
 *   # Real Claude Code, non-interactive (sdk-cli — primary reference):
 *   ANTHROPIC_BASE_URL=http://127.0.0.1:8899 \
 *   _CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL=1 \
 *       claude -p --model claude-sonnet-5 "Reply with exactly: OK"
 *
 *   # Real Claude Code, interactive TUI (research / contingency only):
 *   ANTHROPIC_BASE_URL=http://127.0.0.1:8899 \
 *   _CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL=1 \
 *       python3 scripts/drive-claude-interactive.py --model claude-sonnet-5
 *
 *   # Pi through this extension (load redirect *before* the auth extension):
 ...
 *   pnpm run verify:fingerprint -- <out>
 */
