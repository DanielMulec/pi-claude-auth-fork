...
> **Fork status (v0.6.0):** maintained fork of upstream `pi-claude-auth@0.1.3` (last upstream release 2026-06-04).
> Changes vs upstream:
>
...
> - **`cch` verified against live Claude Code 2.1.267 and 2.1.270** — the recovered seed (`4d659218e32a3268`) plus Claude Code's hash view (empty `model`, dropped `max_tokens`/`fallbacks`/`fallback_credit_token`) reproduces native `cch` on live captures
> - **The captured beta set is merged into pi's, never substituted for it** — declaring it as a provider header silently deleted the betas pi derives from a model's compat flags, which broke per-message effort on `claude-fable-5-1` / `claude-opus-5`
